...
# ╔═╡ 380dfbd9-20e2-4bbf-814b-d9c8d4659025
begin
	using LinearAlgebra
	using Plots
	using Markdown
...
begin
	const L = 0.1       #rigid bar = 10 cm
	const omega = 2.0   #ang vel = 2 rad/s
end;
...
Piston 1 path; this restrains piston 1 to couple the motion of piston 1 by y=x.

$y_1 - x_1 = 0$

Piston 1 orientation; this prevents piston 1 from rotating, as this is a slider, there should only be translational motion.

$\theta_1 - \pi/4 = 0$

Similarly, Piston 2 path and orientation are defined as follows.

$y_2 + x_2 = 0$
$\theta_2 + \pi/4 = 0$

The following four constraints ensure that the respective horizontal (x) and  vertical (y) coordinates of piston 1 and piston 2 match the ends of the rigid bar.

Piston 1 Pin:

$x_1 - x_3 + \frac{L}{2} \cos\theta_3 = 0$

$y_1 - y_3 + \frac{L}{2} \sin\theta_3 = 0$

Piston 2 Pin:

$x_2 - x_3 - \frac{L}{2} \cos\theta_3 = 0$

$y_2 - y_3 - \frac{L}{2} \sin\theta_3 = 0$

The last constraint represents constant angular velocity over time.

$\theta_3 - 2t = 0$

See the code block below for definition of all 9 constraint equations.
"""

# ╔═╡ dfa51c70-1455-4ae2-a355-4de19070eee6
function C(q, t)
...
# ╔═╡ ae01f77e-b4ea-47de-b29c-061c052a2011
function Cq(q, t)
    x1, y1, th1, x2, y2, th2, x3, y3, th3 = q
    J = zeros(9, 9)
    ...
    J[5, 1] = 1.0; J[5, 7] = -1.0; J[5, 9] = -(L/2) * sin(th3)
    J[6, 2] = 1.0; J[6, 8] = -1.0; J[6, 9] =  (L/2) * cos(th3)
    
    J[7, 4] = 1.0; J[7, 7] = -1.0; J[7, 9] =  (L/2) * sin(th3)
    J[8, 5] = 1.0; J[8, 8] = -1.0; J[8, 9] = -(L/2) * cos(th3)
    
    J[9, 9] = 1.0
    return J
end

# ╔═╡ cbd82113-9d3b-4fcd-b9f3-3ba98673ce2c
function Ct(q, t)
    ct = zeros(9)
    ct[9] = -omega
    return ct
...
for t in t_span
    # 1. Newton-Raphson Position Solver
    for iter in 1:20
        err = C(q_current, t)
        if norm(err) < 1e-9
            break
        end
        q_current += Cq(q_current, t) \ (-err)
    end
    
    # 2. Solve for Velocity (C_q * q_dot = -C_t)
    J = Cq(q_current, t)
    qdot = J \ (-Ct(q_current, t))
    
    # 3. Solve for Acceleration (C_q * q_ddot = Gamma)
...
- Our model specifically could be adapted to model a radial airplane engine like shown below. All 5 of the pistons are constrained similarly to the sliders above. They are joined by a pivot on the crank shaft that travels on a set radius. In many ways this is the same model as above with comparable constraints, but includes additional relative coordinate systems and equations. The linear displacements of the pistons could be used to calculate the compression ratios and volumetric displacements. These numbers along with the mass air flow would allow for calculations of the power and torque generated by each revolution.
 -- Fun fact: Pratt and Whitney's first engine, The Wasp, was a radial engine!
...
begin
	...
	rod_length = 3.0
	cylinder_length = 4.0
	engine_angles = range(0, 2π, length = number_of_cylinders + 1)[1:end-1]
	cylinder_directions = [[cos(angle), sin(angle)] for angle in engine_angles]
end

# ╔═╡ 96abe86c-8c00-4c67-95b3-cbb42982cc99
function piston_positions(crank_angle, crank_radius, rod_length, cylinder_directions)
    crank_pin = crank_radius .* [cos(crank_angle), sin(crank_angle)]
    crank_radius_squared = crank_radius^2
    piston_points = Vector{Vector{Float64}}(undef, length(cylinder_directions))
    for (index, axis_unit) in pairs(cylinder_directions)
        projection = crank_pin[1]*axis_unit[1] + crank_pin[2]*axis_unit[2]
        slider_distance = projection + sqrt(rod_length^2 - crank_radius_squared + projection^2)
        piston_points[index] = slider_distance .* axis_unit
    end
    return crank_pin, piston_points
end

# ╔═╡ 6c92b0d8-96fd-4c22-bf28-b7a984edc77d
begin
    notebook_folder = dirname(@__FILE__)
    gif_path = joinpath(notebook_folder, "radial_engine.gif")

    plot_limits = (-5, 5)
    animationengine = @animate for crank_angle in range(0, 2π; length=120)
        crank_pin, piston_points = piston_positions(crank_angle, crank_radius, rod_length, cylinder_directions)
        plot(xlim=plot_limits, ylim=plot_limits, aspect_ratio=:equal, legend=false, axis=false)
        for axis_unit in cylinder_directions
            plot!([0, cylinder_length*axis_unit[1]], [0, cylinder_length*axis_unit[2]], lw=6, c=:gray)
        end
        scatter!([0.0], [0.0], ms=8, c=:black); scatter!([crank_pin[1]], [crank_pin[2]], ms=7, c=:red)
        for piston_point in piston_points
            plot!([crank_pin[1], piston_point[1]], [crank_pin[2], piston_point[2]], lw=3, c=:gold)
            scatter!([piston_point[1]], [piston_point[2]], ms=10, c=:steelblue)
        end
    end
    gif(animationengine, gif_path; fps=30)
    gif_path
	
	  using Base64, HypertextLiteral
    gif_bytes = read(gif_path)
    gif_b64 = base64encode(gif_bytes)

    @htl("""<img src="data:image/gif;base64,$gif_b64" style="max-width:500px; width:100%; height:auto;">""")
end
...
manifest_format = "2.0"
project_hash = "ef00cc630749f9255a52cb0d829d137559878d05"
